...
- **Reversing a verdict requires a new observation.** If rereading the same data changed the conclusion, the verdict did not change — the first one picked the wrong grounds. Say **what was wrong**, not that you reversed. Do not judge the same target three times in a day: one refining fund went conditional -> low priority -> conditional inside a session, and that was the grounds wobbling, not new evidence arriving. **When the same objection surfaces twice, stop amending the verdict and amend the rule.**
