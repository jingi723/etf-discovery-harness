...
레벨(손절·익절)은 **거래 수단이 아니라 기초 지수 기준으로 적는다.** 레버리지 ETF는 음의 복리로 경로 의존이라 같은 지수 레벨에서도 가격이 매번 다르다. SOXL/SOXS로 매매하더라도 레벨은 전부 SOXX 기준으로 쓴다.

포맷과 문구 규칙은 `etf-report-templates`, 금지 표현은 `etf-compliance-rules`를 따른다.
