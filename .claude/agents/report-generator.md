...
2. `final_etf_decision.md` follows the template's ten-section order exactly. **Section 9, "What this supports deciding", is mandatory** (what you can judge / what this cannot settle / data still to obtain / next actions / why this is not a recommendation, plus the closing note). Saying clearly how far ETF Research Agent helps and where the user's own circumstances take over is the reason this report exists.
